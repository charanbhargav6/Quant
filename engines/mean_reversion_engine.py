...
class MeanReversionEngine:
    ...
    def __init__(self):
        ...
        self.rsi_period    = 14
        self.rsi_oversold  = 30     # Classic oversold threshold (was 32 — docstring said 30)
        self.rsi_overbought = 70    # Classic overbought threshold (was 68)
    ...
    def analyze(self, symbol: str, df: pd.DataFrame,
        ...
        if close <= bb_lower * 1.003:  # within 0.3% of lower band
            if rsi_val <= self.rsi_oversold:
                ...
                if not choch.get("bullish_choch", False):
                    return {"signal": None, "reason": "MR long failed: No bullish 15M CHoCH"}
        ...
        if close >= bb_upper * 0.997:
            if rsi_val >= self.rsi_overbought:
                ...
                if not choch.get("bearish_choch", False):
                    return {"signal": None, "reason": "MR short failed: No bearish 15M CHoCH"}
