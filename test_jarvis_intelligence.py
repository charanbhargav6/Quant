"""
JARVIS Intelligence Test Suite
================================
...
def call_gemini(prompt: str, system: str = "") -> str:
    ...
    if resp.status_code != 200:
        return f"ERROR {resp.status_code}: {resp.json().get('error',{}).get('message','Unknown')}"
...
# ── TEST 2: Geopolitical Trade Monitoring ─────────────────────────────────────
open_position = {
    "symbol": "XAUUSD",
    "direction": "BUY",
    "entry": 2310.50,
...
# ── TEST 3: TP Extension on Favorable Geopolitics ─────────────────────────────
...
# ── TEST 5: New Strategy from Plain-English Description ────────────────────────
