...
class PaperTradingEngine:
    ...
    def __init__(self):
        ...
        profile = os.environ.get("CRAVE_PROFILE", "").strip()
        suffix = f"_{profile}" if profile else ""
        self._state_file = STATE_DIR / f"crave_paper_state{suffix}.json"
    ...
    def simulate_fill(self, validated: dict, live_price: float) -> dict:
        ...
        direction = validated.get("direction", validated.get("action", "")).lower()
    ...
    def execute(self, validated: dict, live_price: float) -> dict:
        """Simulate an entry and mirror it into the normal position tracker.

        The live loop and paper loop intentionally share the same validated
        signal. Only the final execution boundary differs, which keeps paper
        results representative of the strategy and risk gates.
        """
        fill = self.simulate_fill(validated, live_price)
        if fill.get("status") == "pending":
            return fill

        fill_price = float(fill.get("fill_price", live_price))
        lot_size = float(validated.get("lot_size", 0) or 0)
        if lot_size <= 0:
            return {"status": "failed", "reason": "Validated signal has no positive lot_size"}
        ...
        trade_id = positions.open(trade)
        logger.info(
            f"[Paper] FILLED | {trade['symbol']} {trade['direction'].upper()} "
            f"@ {fill_price} | size={lot_size} | order_type={fill.get('order_type', 'market')}"
        )
        return {
            "status": "paper_filled",
            "trade_id": trade_id,
    ...
    def close_position(self, trade_id: str, exit_price: float, reason: str = "manual") -> dict:
        """Close a simulated position and settle its R-multiple."""
        from core.position_tracker import positions
        pos = positions.get(trade_id)
        if not pos:
            return {"status": "failed", "reason": f"Paper position {trade_id} not found"}

        entry = float(pos.get("entry_price", 0) or 0)
        sl = float(pos.get("original_sl", pos.get("current_sl", 0)) or 0)
        risk = abs(entry - sl)
        if entry <= 0 or risk <= 0:
            return {"status": "failed", "reason": "Paper position has invalid entry/stop"}

        if pos.get("direction") in ("buy", "long"):
            r_multiple = (float(exit_price) - entry) / risk
        else:
            r_multiple = (entry - float(exit_price)) / risk

        outcome = "manual_close" if reason == "manual" else reason
        closed = positions.close(trade_id, float(exit_price), r_multiple, outcome)
        if closed is None:
            return {"status": "failed", "reason": f"Paper position {trade_id} could not close"}
        return {
            "status": "paper_closed",
            "trade_id": trade_id,
    ...
    def check_readiness(self, backtest_win_rate: float = None,
        ...
        paper_mode = bool(cfg.get("enabled", True))
        chk(
            "Exchange API keys set (live only)",
            True if paper_mode else has_keys,
            "✅ Not required in paper mode" if paper_mode else
            ("✅ Found" if has_keys else "❌ Missing — add broker credentials to .env"),
        )
...
# Deliberately do not instantiate the engine at import time. Callers should
# use get_paper_engine() so status/readiness commands remain side-effect free
# until they explicitly need paper state.
